...
const EditBoat = () => {
  ...
  const [imagesFile, setImagesFile] = useState<(File | string)[]>([]);
  ...
  const { isLoading, error } = useQuery<Boat | null>(
    ["boat", id],
    () => getSingleBoat(id || ""),
    {
      enabled: !!id,
      ...
      onSuccess: (boatData) => {
        if (boatData) {
          formik.setValues({
            ...
            boarding: boatData.boarding || "",
            boardingTime: boatData.boardingTime || "",
            ...
            structure: boatData.structure || [],
            images: boatData.images || [], // As URLs das imagens existentes
          });
          setDetails(boatData.details || []);
          setScripts(boatData.scripts || []);
          setStructure(boatData.structure || []);
          setImagesFile(boatData.images || []);
        }
      },
      onError: (err) => {
  ...
  const formik = useFormik<Boat>({
    ...
    onSubmit: async (values, { setSubmitting, setFieldError }) => {
      ...
      try {
        ...
        console.log(e);
        setAlertMessage("Erro ao atualizar embarcação. Tente novamente.");
        setAlertType("error");
        setOpenAlert(true);
        setSubmitting(false);
        setFieldError(
          "general",
          "Erro ao atualizar embarcação. Tente novamente."
        );
        console.error("Erro ao atualizar embarcação:", e);
      }
      setLoading(false);
    },
  });
  ...
  return (
    <>
      <h2 className="text-primary mt-4">
        Atualizando os dados de sua embarcação.
      </h2>
